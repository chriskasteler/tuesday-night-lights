    ...
    try {
        ...
        if (!teamPlayers) {
            ...
            if (window.teamPlayersMap) {
                const exactMatch = window.teamPlayersMap[teamName];
                const similarTeams = Object.keys(window.teamPlayersMap).filter(name => 
...
window.setStroke = function setStroke(player, hole, strokeType) {
    try {
        ...
        // Use the existing stroke storage system but make it cell-based
        ...
        // Update visual indicators using existing functions
